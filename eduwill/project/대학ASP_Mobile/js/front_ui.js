...
$(function(){
	...
	// 교육과정 열고닫기
	$('.course_box .select_box a').each(function(i){
		$(this).click(function(){
			if ( $('.course_box .item_box').eq(i).is(":hidden") ){
				$('.course_box .item_box').hide();
				$('.course_box .item_box').eq(i).show();
				$('.course_box .select_box').removeClass('open');
	...
	// 전체선택 및 해제
	/*$('.item_list .all_check input').each(function(i){
		$(this).click(function(){
			if ( $(this).attr("checked") == "checked" ) {
				$(".sample input").attr("checked", false);
				$(".item_list ul").eq(i).find("input").attr("checked", true);
			} else {
				$(".sample input").attr("checked", false);
			}
		});
